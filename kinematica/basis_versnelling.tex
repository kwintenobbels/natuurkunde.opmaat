...
\begin{definition}
	...
	De \textbf{versnellingsvector \(\vec{a}\)} van een bewegend punt wordt gedefinieerd als de afgeleide van de snelheidsvector naar de tijd:
	...
	\[
		\vec{a} = \frac{d\vec{v}}{dt} = \frac{d}{dt}(v_x \cdot \vec{e}_x + v_y \cdot \vec{e}_y) = \frac{dv_x}{dt} \cdot \vec{e}_x + \frac{dv_y}{dt} \cdot \vec{e}_y = \vec{a}_x + \vec{a}_y
	\]
	
	De grootte van de totale versnelling wordt genoteerd met het symbool \(a\) en is gelijk aan 
	\[
	a = \| \vec{a}\| = \sqrt{(\vec{a}_x)^2 + (\vec{a}_y)^2}
	\]
...
\begin{remark}
	In bovenstaande figuur werd 'in het algemene geval' een versnellingsvector getekent als vector die aangrijpt op de puntmassa. 
